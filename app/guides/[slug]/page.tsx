    ...
    <div className="relative isolate overflow-hidden px-4 pb-24 pt-8 text-[var(--ink)] sm:px-6 lg:px-8 lg:pt-12">
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(articleJsonLd).replace(/</gu, "\\u003c"),
        }}
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[30rem] opacity-60"
        style={{
          backgroundImage:
            "radial-gradient(circle at 12% 8%, color-mix(in srgb, var(--accent) 12%, transparent), transparent 44%)",
        }}
      />
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <article className="mt-10">
          <header className="grid gap-8 border-b border-[var(--line)] pb-10 md:grid-cols-[minmax(0,1fr)_13rem] md:items-end md:gap-16 md:pb-14">
            <div>
              <h1 className="max-w-[18ch] text-[clamp(2.25rem,5vw,3rem)] font-semibold leading-[1.04] tracking-[-0.04em]">
                {guide.title}
              </h1>
              <p className="mt-6 max-w-[44rem] text-lg leading-8 text-[var(--muted)]">
                {guide.description}
              </p>
            </div>
            ...
            <dl className="grid grid-cols-2 gap-4 text-sm md:grid-cols-1 md:border-l md:border-[var(--line)] md:pl-6">
              <div>
                ...
                <dd className="mt-1 font-semibold">{formatDate(guide.updated)}</dd>
              </div>
              <div>
        ...
        {relatedGuides.length > 0 ? (
          <section
            ...
            className="mt-20 border-t border-[var(--line)] pt-10"
          >
            <h2
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-[1.35fr_0.65fr]">
              {relatedGuides.map((related) => (
                ...
                >
                  <span className="font-semibold leading-6">{related.title}</span>
