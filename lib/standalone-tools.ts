...
const zh: ToolDictionary = {
  ...
  "black-level-test": {
    title: "黑位测试",
    seoTitle: "在线黑位测试：检查近黑阶、暗部压黑与纯黑均匀性",
    description: "用纯黑、RGB 1-10 近黑阶和嵌入式暗部图案检查显示器的黑位、阴影细节与暗部压黑。",
    keywords: ["黑位测试", "近黑阶测试", "暗部压黑测试", "PLUGE 测试"],
    shortName: "黑位",
    modeSummary: "3 种黑位图案",
    duration: "约 2 分钟",
    startLabel: "开始黑位测试",
    ...
    howTitle: "怎么做黑位测试",
    preparation: [
      "使用 SDR 模式和 100% 浏览器缩放，把窗口移到要检查的显示器。",
      "保持日常亮度，关闭夜间模式和动态对比度，再把环境光调暗。",
      "先看纯黑均匀性，再数出 RGB 1-10 中能与黑色分开的色块。",
      "最后查看嵌入暗块和细横纹；如果有局部调光，可分别开关测试一次。",
    ],
    lookTitle: "重点看什么",
    observations: [
      { signal: "RGB 1-10 全部融进黑色", meaning: "Gamma、黑位或显卡输出范围可能压掉了暗部细节，也可能是显示器近黑表现有限。" },
      { signal: "纯黑画面整体发灰", meaning: "先排除环境光反射和 RGB Limited 输出；LCD 的原生黑位也会影响观感。" },
      { signal: "暗块可见，但细横纹消失", meaning: "非常接近黑色的细节可能被处理、局部调光或量化精度抹掉。" },
    ],
    limitation: "网页只能输出固定 RGB 图案，不能测量黑色亮度、对比度、面板类型或尼特数，也不能替你判断退换货和保修。",
    faqTitle: "黑位测试常见问题",
    faqs: [
      { question: "应该看到几个近黑阶色块？", answer: "没有适用于所有设备的合格数字。记录你能稳定看见的数量，并在只改一个设置后重新比较。" },
      { question: "为什么要关闭 HDR？", answer: "这个测试使用 SDR RGB 值。HDR 色调映射可能重新分配近黑亮度，让不同浏览器和系统更难直接比较。" },
      { question: "RGB Full 和 Limited 会影响结果吗？", answer: "会。错误的 0-255 与 16-235 范围组合可能抬高黑位或吞掉暗部，先检查显卡、电视和显示器的范围设置是否一致。" },
      { question: "这和漏光测试有什么区别？", answer: "漏光测试主要找边缘亮斑和 IPS Glow；黑位测试还会检查 RGB 1-10 阴影分离和嵌入式暗部细节。" },
    ],
    relatedTitle: "相关工具",
    related: [
...
const de: ToolDictionary = {
  ...
  "black-level-test": {
    title: "Schwarzpegel-Test",
    seoTitle: "Schwarzpegel-Test online: Dunkelstufen und Black Crush prüfen",
    description: "Prüfe Schwarzpegel, Schattendetails und Black Crush mit reinem Schwarz, RGB-Stufen 1-10 und eingebetteten dunklen Mustern.",
    keywords: ["Schwarzpegel Test", "Near Black Test", "Black Crush Test", "PLUGE Test"],
    shortName: "Schwarzpegel",
    modeSummary: "3 Schwarztestbilder",
    duration: "Etwa 2 Minuten",
    startLabel: "Schwarzpegel-Test starten",
    toolLabel: "Online-Werkzeug für den Schwarzpegel-Test",
    howTitle: "So prüfst du den Schwarzpegel",
    preparation: [
      "Nutze SDR und 100 % Browserzoom und verschiebe das Fenster auf den gewünschten Monitor.",
      "Lass die normale Helligkeit eingestellt, schalte Nachtmodus und dynamischen Kontrast aus und dimme den Raum.",
      "Prüfe zuerst reines Schwarz und zähle dann die RGB-Stufen 1-10, die du von Schwarz trennen kannst.",
      "Suche zuletzt die eingebetteten Felder und feinen Streifen. Teste Local Dimming bei Bedarf einmal an und einmal aus.",
    ],
    lookTitle: "Darauf solltest du achten",
    observations: [
      { signal: "RGB 1-10 verschmilzt vollständig mit Schwarz", meaning: "Gamma, Schwarzpegel oder der GPU-Ausgabebereich können Schattendetails abschneiden. Auch das Display selbst kann nahe Schwarz begrenzen." },
      { signal: "Die gesamte schwarze Fläche wirkt grau", meaning: "Schließe Raumlichtreflexionen und einen falschen Limited-Ausgabebereich aus. Auch der native LCD-Schwarzpegel spielt eine Rolle." },
      { signal: "Dunkle Felder sind sichtbar, feine Streifen aber nicht", meaning: "Sehr dunkle Feindetails können durch Bildverarbeitung, Local Dimming oder Quantisierung verloren gehen." },
    ],
    limitation: "Die Webseite gibt feste RGB-Testbilder aus. Sie misst weder Schwarzluminanz noch Kontrast, Paneltyp oder Nits und entscheidet nicht über Rückgabe oder Garantie.",
    faqTitle: "Häufige Fragen zum Schwarzpegel-Test",
    faqs: [
      { question: "Wie viele Dunkelstufen sollte ich sehen?", answer: "Es gibt keine allgemeine Bestehenszahl. Notiere die stabil sichtbaren Felder und vergleiche erneut, nachdem du nur eine Einstellung geändert hast." },
      { question: "Warum sollte HDR ausgeschaltet sein?", answer: "Der Test nutzt SDR-RGB-Werte. HDR-Tone-Mapping kann die Dunkelstufen neu verteilen und Vergleiche zwischen Systemen erschweren." },
      { question: "Beeinflussen RGB Full und Limited das Ergebnis?", answer: "Ja. Eine falsche Kombination aus 0-255 und 16-235 kann Schwarz anheben oder Schatten abschneiden. GPU, Fernseher und Monitor müssen denselben Bereich verwenden." },
      { question: "Was ist der Unterschied zum Backlight-Bleeding-Test?", answer: "Der Bleeding-Test sucht vor allem helle Ränder und IPS Glow. Dieser Test prüft zusätzlich RGB 1-10 und eingebettete Schattendetails." },
    ],
    relatedTitle: "Passende Werkzeuge",
    related: [
      { href: "/monitor-color-calibration", label: "Monitorkalibrierung" },
      { href: "/tests/backlight-bleed", label: "Backlight-Bleeding-Test" },
      { href: "/tests/grayscale", label: "Graustufen-Test" },
